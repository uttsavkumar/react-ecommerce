...
const KidProduct = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/api/kid').then((res) => {
      setData(res.data)
    })
    axios.get('http://localhost:8000/api/kidcat').then((res) => {
      setCat(res.data)
    })
  }, [])
      ...
      <Grid container>
        <Grid item xs={3} sx={{ borderRight: '.1px solid #f0efef9c', position: 'sticky', zIndex: 10, top: 0 }} >
          <Box sx={{ height: '100vh', paddingLeft: 7, pt: 1 }}>
            <Typography sx={{ fontSize: 25, fontWeight: 500 }}>Kid's Shoes({data.length})</Typography>
